...
export const deleteSharedSummarySchema = {
  type: "object",
  properties: {
...
export async function deleteSharedSummary({ id, date, dayOfWeek, limit, timeRange }: { id?: string; date?: string; dayOfWeek?: string; limit?: number; timeRange?: { start: string; end: string } }): Promise<{ content: Array<{ type: string; text: string }>; isError?: boolean }> {
  console.error(`Deleting shared summary with params: ${JSON.stringify({ id, date, dayOfWeek, limit, timeRange })}`);
  ...
  try {
    ...
    if (!targetId) {
      console.error("No ID provided, searching for notes based on filters");
      ...
      console.error(`Taking top ${n} results`);
      ...
      console.error(`Selected note with ID: ${targetId} for deletion`);
    }
    
    console.error(`Deleting note with ID: ${targetId}`);
    ...
    if (!resDelete.ok) {
      ...
      console.error(`HubSpot API error: HTTP-Code: ${resDelete.status}, Message: ${deleteData.message}`);
      throw new Error(`HTTP-Code: ${resDelete.status}\nMessage: ${deleteData.message}`);
    }
    
    console.error(`Summary deleted successfully. Engagement ID: ${targetId}`);
